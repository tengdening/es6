...
```
	...
	false;
```
* isunit: 如果一个值是带指定单位的数字,返回true,否则返回false。
```
	isunit(15px,px);
	isunit(15555,px);
```
...
```
	...
	false;
```
* isruleset: 如果一个值是规则集合,返回true,否则返回false。
```
	@rules: {
	    color: red;
	}
...
```
	rgba(255,0,0,0.5);
```
* argb: 创建格式为 #AARRGGBB 的十六进制 (hex representation) 颜色 (注意不是 #RRGGBBAA ！)。
...
```
	#805a1794;
```
* hsl: 通过色相,饱和度,亮度三种值 (HSL) 创建不透明的颜色对象。
> 参数1：整数0-360表示度数。参数2: 整数 0-100% 或是数字 0-1。参数3: 整数 0-100% 或是数字 0-1
```
	hsl(90, 100%, 50%);
```
> 输出
```
	#80ff00;
```
* hsla: 通过色相,饱和度,亮度,以及透明度四种值 (HSL) 创建透明的颜色对象。
```
	hsla(90, 100%, 50%, 0.5);
```
> 输出
```
	rgba(128, 255, 0, 0.5);
```
* hsv: 通过色相、饱和度、色调三种值创建不透明的颜色对象。
> 参数1：整数 0-360 代表度数。参数2：百分比 0-100% 或数字 0-1。参数3：百分比 0-100% 或数字 0-1
```
	hsv(90, 100%, 50%);
```
> 输出
```
	#408000;
```
* hsva: 通过色相,饱和度,色调,以及透明度四种值创建透明的颜色对象。
```
	hsva(90,100%,50%,0.5);
```
> 输出
...
```
	90;
```
* saturation: 从HSL色彩空间中提取颜色对象的饱和度值。
```
...
```
	90;
```
* hsvsaturation: 从HSV色彩空间中提取颜色对象的色相值。
```
...
```
	44%;
```
* luminance: 计算颜色对象luminance的值(亮度的百分比表示法)。
```
